    ...
    <div className={`bg-panel border border-border rounded-card ${className}`}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2 text-txt text-sm font-medium">
          ...
          <span>我的收藏</span>
          <span className="text-txt-secondary text-xs">({items.length})</span>
        </div>
              ...
              >
                ...
                {onRemove && (
                  <button
                    ...
                    disabled={isRemoving}
                    className="text-txt-secondary/40 hover:text-danger transition-colors disabled:opacity-50 shrink-0"
                    title="取消收藏"
                  >
